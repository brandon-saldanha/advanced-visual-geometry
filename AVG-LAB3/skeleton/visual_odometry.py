...
class VisualOdometry:
    ...
    def find_matches(self, incoming_frame: np.ndarray) -> List[cv2.DMatch]:
        ...
        if not self.src_kpts:
            self.src_kpts = kpts 
            self.src_desc = desc  # TODO
            return []
        else:
            # find matches between incoming kpts and src kpts
            self.incoming_kpts = kpts
            self.incoming_desc = desc # TODO
    ...
    def compute_relative_transform(self, matches: List[cv2.DMatch], update_src_frame: bool) -> \
        ...
        # compute homography
        mat_H, mask = cv2.findHomography(
            src_pts, incoming_pts, cv2.RANSAC, self.homography_ransac_threshold
        )

        if mat_H is None or mask is None:
            return np.array([]), np.array([])
        ...
        if num_candidates > 1:
            ...
            for i, n in enumerate(normals):
                ...
                d_over_z = (src_pts_normalized @ n).flatten() # TODO: result should be an array of <float: num_inliers>
                # check if any ratio is negative
                is_valid = not np.any(d_over_z < 0)  # TODO: True if every inlier has positive depth, False otherwise
                if not is_valid:
            ...
            if len(rots) > 1:
                ...
                n0 = normals[0] / np.linalg.norm(normals[0])
                n1 = normals[1] / np.linalg.norm(normals[1])
                nref = self.plane_normal_src / np.linalg.norm(self.plane_normal_src)
                angle_0 = float(np.arccos(np.clip((n0.T @ nref).item(), -1.0, 1.0)))  # TODO: compute angle between 1st candidate of normal vector with self.plane_normal_src
                angle_1 = float(np.arccos(np.clip((n1.T @ nref).item(), -1.0, 1.0))) # TODO: compute angle between 2nd candidate of normal vector with self.plane_normal_src
        ...
        if rots:
            ...
            if self.plane_d_src < 0:
                ...
                self.plane_d_src = float(self.plane_normal_src.T @ self.plane_origin_src)  # TODO: compute distance from the 1st camera frame C0 to the plane
            ...
            if update_src_frame:
                ...
                self.plane_normal_src = rots[0] @ normals[0]  # TODO
                ...
                depth = self.plane_d_src / d_over_z # TODO: compute depth of inlier using d and normalize coordinate, result should be
                # TODO: (continue) an array of <float: num_inliers,>

                # find 3D coordinate in src frame of these inliers
                src_pts_3d = src_pts_normalized * depth.reshape(-1, 1)  # TODO: compute 3d coordinate in src camera frame for every inlier,
                # TODO: (continue) result should be a matrix of shape <num_inliers, 3>

                # map these points to incoming frame
                incmoing_pts_3d = (rots[0] @ src_pts_3d.T + trans[0]).T # TODO: result should have shape <num_inliers, 3>

                # compute new d by averaging projection of every point onto plane's normal vector
                self.plane_d_src = float(np.mean(incmoing_pts_3d @ self.plane_normal_src)) # TODO
